...
if __name__ == "__main__":
    ...
    # Currency management
    usd = UniversalCurrency(100, "USD", exchange_rates)
    eur = UniversalCurrency(100, "EUR", exchange_rates)

    converted_to_eur = usd.convert_to("EUR")
    print(converted_to_eur)

    converted_to_usd = eur.convert_to(usd)
    print(converted_to_usd)

    usd.add_currency("INR", 74.0)
    inr = UniversalCurrency(1000, "INR", exchange_rates)

    converted_to_inr = usd.convert_to("INR")
    print(converted_to_inr)

    usd.update_exchange_rate("INR", 75.0)
    converted_to_inr_updated = usd.convert_to("INR")
    print(converted_to_inr_updated)

    usd.add_currency("YEN", 0.35)
    inr = UniversalCurrency(100, "YEN", exchange_rates)

    converted_to_yen = usd.convert_to("YEN")
    print(converted_to_yen)

    # usd.update_exchange_rate("YEN", 75.0)
    # converted_to_inr_updated = usd.convert_to("YEN")
    # print(converted_to_inr_updated)

    # Domestic transactions
    domestic_sender = Transaction(500, "USD")
    domestic_receiver = Transaction(300, "USD")

    print(domestic_sender)
    print(domestic_receiver)

    domestic_sender.send_money(domestic_receiver, 100)
    print(domestic_sender)
    print(domestic_receiver)

    # International transactions
    international_sender = InternationalTransaction(
        1000, "USD", exchange_rates)
    international_receiver = InternationalTransaction(
        500, "EUR", exchange_rates)

    print(international_sender)
    print(international_receiver)

    international_sender.send_money(international_receiver, 200, "EUR")
    print(international_sender)
    print(international_receiver)
